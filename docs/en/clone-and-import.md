...
`git clone https://github.com/ByteLegendQuest/java-add-audit-log`

A project directory named `java-add-audit-log` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-add-audit-log` directory which is created just now.
