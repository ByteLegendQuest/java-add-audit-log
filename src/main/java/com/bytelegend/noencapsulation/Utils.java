...
public class Utils {
    public static List<Person> filterPeopleWithSalaryMoreThan1(List<Person> people, int salary) {
        ...
        return people.stream().peek(person -> Logger.logAccessToSalaryOfPerson(person.name)).filter(p -> p.salary > salary).collect(Collectors.toList());
    }
    ...
    public static Map<String, Integer> getNameToSalaryMap1(List<Person> people) {
        return people.stream().peek(person -> Logger.logAccessToSalaryOfPerson(person.name)).collect(Collectors.toMap(p -> p.name, p -> p.salary));
    }
