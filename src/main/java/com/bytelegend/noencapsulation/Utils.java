...
public class Utils {
    public static List<Person> filterPeopleWithSalaryMoreThan1(List<Person> people, int salary) {
        return people.stream().filter(p -> {Logger.logAccessToSalaryOfPerson(p.name);
            return p.salary > salary;}).collect(Collectors.toList());
    }
    ...
    public static Map<String, Integer> getNameToSalaryMap1(List<Person> people) {
        return people.stream().collect(Collectors.toMap(p -> p.name, p -> {Logger.logAccessToSalaryOfPerson(p.name);
            return p.salary;}));
    }
